...
// Get the email key from script tag's data-question-id attribute
function getEmailKeyFromScript() {
  const scriptTag = document.querySelector('script[src*="surface.js"]');
  if (scriptTag && scriptTag.hasAttribute("data-question-id")) {
    return scriptTag.getAttribute("data-question-id");
  }
  return "";
}

/**
 * SurfaceEmbed class handles the core functionality for embedding Surface forms
 * in different display modes (inline, popup, slideover, widget, input-trigger)
 */
class SurfaceEmbed {
  /**
   * @param {string} src - The URL of the Surface form
   * @param {string} embed_type - Type of embedding ('inline', 'slideover', 'popup', 'widget', 'input-trigger')
   * @param {string} target_element_class - CSS class to target for embedding
   * @param {Object} options - Configuration options including widget styles and email key
   */
  constructor(src, embed_type, target_element_class, options = {}) {
    this.enableWidget = options.enableWidget || false;
    this.emailKey = options.emailKey || getEmailKeyFromScript();
  ...
  /**
   * Embeds the Surface form inline within elements matching target_element_class
   */
  embedInline() {
    if (
      this.inline_embed_references == null ||
  ...
  /**
   * Shows the popup form with optional parameters
   * @param {Object} options - Optional parameters to pass to the form
   */
  ...
  /**
   * Embeds the Surface form as a popup overlay
   */
  embedPopup() {
    if (this.surface_popup_reference == null) {
      this.log(
  ...
  /**
   * Embeds the Surface form as a slideover panel
   */
  embedSlideover() {
    if (this.surface_popup_reference == null) {
      this.log(
  ...
  /**
   * Adds the floating widget button to the page
   */
  addWidgetButton() {
  ...
  /**
   * Embeds the Surface form as a widget (floating button that opens popup)
   */
  embedWidget() {
  ...
  /**
   * Embeds the Surface form with input trigger functionality
   * Monitors email input forms and triggers the popup on valid submission
   */
  embedInputTrigger() {
...
/**
 * SurfaceFormEmbed class handles the initialization and embedding of Surface forms
 * based on script tag attributes
 */
class SurfaceFormEmbed {
  /**
   * @param {HTMLElement} childScriptTag - The script tag containing Surface form configuration
   */
  constructor(childScriptTag) {
    this.childScriptTag = childScriptTag;
    this.formSrc = childScriptTag.getAttribute("data-form-src");
    this.embedType = childScriptTag.getAttribute("data-embed-type");
    this.targetElementClass = childScriptTag.getAttribute(
      "data-target-element-class"
    );
    this.options = {
      emailKey: childScriptTag.getAttribute("data-question-id")
        ? `${childScriptTag.getAttribute("data-question-id")}_emailAddress`
        : null,
    };
    this.popupSize = childScriptTag.getAttribute("data-popup-size");
  }

  /**
   * Initializes and embeds the Surface form
   */
  embed() {
    ...
    cSurfaceForm.popupSize = this.popupSize;
    document.addEventListener("click", cSurfaceForm.embedSurfaceForm(), {
      once: true,
    });
    document.addEventListener("scroll", cSurfaceForm.embedSurfaceForm(), {
      once: true,
    });
  }
}
